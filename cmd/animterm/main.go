...
func main() {
	mode := flag.String("mode", "cybercube", "cybercube | rain | spectrum | cloud | starfield | orbit")
	...
	case "orbit", "hud", "core":
		...
		orbit.Run(cfg)
	default:
		fmt.Printf("unknown mode %q (expected cybercube | rain | spectrum | cloud | starfield | orbit)\n", *mode)
	}
}
